...
public class ChildProfileStore extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_EVENTS + " ("
                ...
                + "FOREIGN KEY(" + COLUMN_CHILD_ID + ") REFERENCES "
                + TABLE_PROFILES + "(" + COLUMN_ID + ") ON DELETE CASCADE"
                + ")");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 2) {
            db.execSQL("ALTER TABLE " + TABLE_PROFILES
                    + " ADD COLUMN " + COLUMN_GENDER
                    + " TEXT NOT NULL DEFAULT 'غير محدد'");
        }
    ...
    private void seedSoundsAndVideosData(SQLiteDatabase db) {
        ...
        insertVideo(db, "أصدقائي وأنا", "قصة عن الصداقة الحلوة", "صداقة",
                "thumb_friends", "#F49AC2", "friends_video", "4:26");

        insertVideo(db, "تنمية المشاعر", null, "مشاعر",
                "thumb_emotions", "#4DD0C4", "emotions_video", "3:45");

        insertVideo(db, "أحلام جميلة", null, "نوم",
                "thumb_dreams", "#5DADE2", "dreams_video", "6:00");

        insertVideo(db, "أنا شجاع", null, "لعبة",
                "thumb_brave", "#F0932B", "brave_video", "4:00");
    }

    private void insertSound(SQLiteDatabase db, String title,
                             String icon, String file,
    ...
    private void insertVideo(SQLiteDatabase db, String title,
                             String subtitle, String category,
                             String thumbnail, String bgColor,
                             String file, String duration) {
        ...
        try (Cursor cursor = getReadableDatabase().query(
            ...
            while (cursor.moveToNext()) {
                list.add(new SoundItem(
                        ...
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_SOUND_TITLE)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_SOUND_ICON)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_SOUND_FILE))
                ));
            }
        }
    ...
    public long addChatMessage(long childId, String messageText,
                               boolean isUser) {
        ...
        try (Cursor cursor = getReadableDatabase().query(
            ...
            while (cursor.moveToNext()) {
                ...
                String text = cursor.getString(
                        cursor.getColumnIndexOrThrow(COLUMN_MESSAGE_TEXT));

                boolean isUser = cursor.getInt(
                        cursor.getColumnIndexOrThrow(COLUMN_IS_USER)) == 1;
    ...
    public List<VideoItem> getVideosByCategory(String category) {
        return queryVideos(
                COLUMN_VIDEO_CATEGORY + " = ?",
                new String[]{category}
        );
    }

    private List<VideoItem> queryVideos(String selection,
        ...
        try (Cursor cursor = getReadableDatabase().query(
                ...
                new String[]{
                        COLUMN_ID,
                        COLUMN_VIDEO_TITLE,
                        COLUMN_VIDEO_SUBTITLE,
                        COLUMN_VIDEO_CATEGORY,
                        COLUMN_VIDEO_THUMBNAIL,
                        COLUMN_VIDEO_BG_COLOR,
                        COLUMN_VIDEO_FILE,
            ...
            while (cursor.moveToNext()) {
                ...
                list.add(new VideoItem(
                        cursor.getLong(
                                cursor.getColumnIndexOrThrow(COLUMN_ID)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_TITLE)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_SUBTITLE)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_CATEGORY)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_THUMBNAIL)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_BG_COLOR)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_FILE)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_VIDEO_DURATION))
                ));
            }
        }
    ...
    public void migrateFromSharedPreferencesIfNeeded(Context context) {
        ...
        String savedProfiles = prefs.getString(
                OLD_KEY_PROFILES,
                null
        );
    ...
    public long addProfile(String name, int age,
        ...
        return getWritableDatabase()
                .insert(TABLE_PROFILES, null, values);
    }
    ...
    public long addBehaviorEvent(long childId,
        ...
        return getWritableDatabase()
                .insert(TABLE_EVENTS, null, values);
    }

    public long addBotMessage(long childId, String text,
                              String mood, long createdAt) {
        ...
        return getWritableDatabase()
                .insert(TABLE_BOT_MESSAGES, null, values);
    }
            ...
            while (cursor.moveToNext()) {
                ...
                messages.add(new BotMessage(
                        cursor.getLong(
                                cursor.getColumnIndexOrThrow(COLUMN_ID)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_TEXT)),
                        cursor.getString(
                                cursor.getColumnIndexOrThrow(COLUMN_MOOD)),
                        cursor.getLong(
    ...
    private int getProfilesCount() {
        ...
        try (Cursor cursor = getReadableDatabase().rawQuery(
                "SELECT COUNT(*) FROM " + TABLE_PROFILES,
                null)) {
    ...
    public boolean hasCompletedEventToday(long childId,
                                          String eventType) {
